...
object Aws4SignerSpec extends Specification {
  ...
  "Aws4Signer" should {
    ...
    "example1" >> new WithApplication {
      ...
      val request =
        WS.url("https://sts.amazonaws.com")
          .withQueryString(
    ...
    "example3 PUT Object" >> new WithApplication {
      ...
      val signedRequest = signer.sign(request, "PUT", body)
    ...
    "example6 url signature" in {
      ...
      val signer = newSigner()
      val url = signer.signUrl("https://examplebucket.s3.amazonaws.com/test.txt", 86400)
  ...
  def newSigner(service: String = "s3", date: Date = defaultDate) =
    new Aws4Signer(credentials, service, "us-east-1") {
